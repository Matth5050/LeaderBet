...
function Scores() {
  ...
  const output = nums.map((element, index) => {
    return  (
    <table className="table table-dark scroll">
      <tbody key={index} className="" >
        <tr><td className="col">{element.scores[0].name}</td><td className="text-end">{element.scores[0].score}</td></tr>
        <tr><td className="col">{element.scores[1].name}</td><td className="text-end">{element.scores[1].score}</td></tr>
      </tbody>
    </table>
  )})
